...
function mountVaultShelf(root, data, options) {
  ...
  function renderContents() {
    ...
    reader.book.notes.forEach(function (note, i) {
      if (needle && !core.matchesQuery(note, needle)) return;
      var li = el("li");
      var b = el("button");
      b.type = "button";
      ...
      if (lit && core.matchesQuery(note, lit)) b.setAttribute("data-match", "1");
  ...
  function renderMeta(note) {
    ...
    /* github#13, design/0027 -- a match with no detail to point at says so instead */
    var reasons = lit ? core.matchReasons(note, lit) : [];
    var unseen = reasons.length > 0 && !reasons.some(function (r) {
      return r.field !== "body" && r.field !== "path";
    });
    ...
    if (unseen) {
      ...
      line.appendChild(el("span", "vs-why", reasons.some(function (r) { return r.field === "body"; })
        ? "matches in the text" : "matches in the path"));
    }
