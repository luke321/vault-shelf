...
/* github#40, design/0026 -- a synthetic wheel does not scroll; these apply it by hand */
...
check("a wheel on the spread stays smooth in every look", async (p) => {
  await p.eval(PUSH_HELPERS);
  /* github#40, design/0026 -- the library scroll's budget, same method */
  /* github#40, design/0026 -- two costs, so measured where the push cannot turn */
  const r = await p.eval(`(async function(){
    var looks = window.VaultShelfCore.LOOKS.map(function (l) { return l.value; });
