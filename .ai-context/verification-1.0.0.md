...
| Lint, typecheck and static release gates | `release.ps1 1.0.0` on `main` at `774390a` (merge of PR #59, `develop` → `main`) ran lint, typecheck and the plugin pre-flight build clean: `typecheck: ok -- tsc --noEmit clean`, `lint: ok -- 0 errors, 0 warnings`, `main.js 213.8kb` built. |
| Full invariant suite / stamp | The first release dry run at `0dbce28` passed 130/133. Manual plaque membership, Reading baseline measurement and index cleanup failed and were repaired below. After repairs and the counter merge, the full suite passed **135/135 in 86 seconds** at `8afede4`, tree `553ea4963488ca1a6e0bd68c0631e792826184bf` (`dist/post-counter-full-suite.log`). This is green 1/2 for that source tree; the final docs/media tree must earn its own stamp. Current stamp policy requires two consecutive full passes; no preparation-worker full run is claimed. The `develop` push (`d838629`, redacting the vault name from this file) earned the tree's **first** green run of 135/135 in 80s. `release.ps1 1.0.0 -DryRun` on `main` at `774390a` (same tree, 5a08a84) earned the **second** consecutive green run of 135/135 in 85s, and the stamp was written: `stamped tree 5a08a84 as passed 2 times in a row`. The real `release.ps1 1.0.0` run then found the tree already stamped and skipped the suite (`-ForceSuite` not used). |
| `release.ps1 1.0.0 -DryRun -AllowAnyBranch` | Initial run stopped on the 130/133 suite result (`dist/release-dryrun-1.0.0.log`). The user retained approved media and instructed continuation after the requested counter change. The final dry run — plain `release.ps1 1.0.0 -DryRun` on `main` at `774390a`, no `-AllowAnyBranch` needed since `main` was already the branch — measured the final committed tree: 135/135, second consecutive green, stamped as above. `plugin/whats-new.md`'s update strip was rendered and visually reviewed before tagging (`node scripts/update-note-check.mjs`, 33/33 passed, `01-strip-up.png`): the strip reads correctly, links `1.0.0` and the feature gallery, shows five accurate bullets, and does not disturb the library beneath it. |
| `release.yml` branch dry run | Passed on `release/1.0.0` at `3a7e484` before the merge (run `34743134158`, 2026-09-13T06:35:48Z, success) — the PUBLIC-repository attestation path is confirmed working, unlike the earlier private-repo failure on `release/0.1.0`. |
| Published assets and attestation verification | Tag `1.0.0` pushed from `main` at `774390a`; workflow run [`34744485442`](https://github.com/luke321/vault-shelf/actions/runs/34744485442) succeeded, publishing release [`1.0.0 - Vault Shelf`](https://github.com/luke321/vault-shelf/releases/tag/1.0.0). Downloaded and hashed all three assets — they match the release API's own digests exactly: `main.js` `b310e6ee0e81ba898956a0e4ba145926d74f32239d98c29512a21b3f0bcf23d8`, `manifest.json` `e3b528b6611be8ebedd3e7a3d9ad58aee4546ec44f2bcc255051dc29268e36f6`, `styles.css` `2a21bd21844ef79f5222783be9435781f7e3167b6313bf0a4a587680a8b96105`. `gh attestation verify <file> --repo luke321/vault-shelf` succeeded (exit 0) for all three, each carrying a Sigstore/SLSA provenance statement built from `refs/tags/1.0.0` at commit `774390a`. |
