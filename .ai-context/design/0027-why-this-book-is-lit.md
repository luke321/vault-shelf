...
`core.matchReasons(note, needle)` returns the surfaces the needle was found in —
`title`, `tag`, `person`, `folder`, `path`, `body` — and `renderMeta` marks the needle **where it
...
A note whose only match is the body or the rest of the path has nothing a reader could be pointed
at, so the line **says so in words** (`matches in the text`) rather than marking nothing and
leaving the question open. That phrase disappears on its own the day those surfaces leave the
rule.
...
> vault across five needles.

Measured: **4,938 notes × 5 needles, 1,700 marked, 1,700 with a reason, 0 disagreements.**
...
- **What the query matches.** `matchesQuery` is unchanged, body and path included; that rule is
  `github#58`'s.
