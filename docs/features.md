...
One reason is not written on the note at all. The search also reads the **name printed on a
spine**, so a note can be drawn forward by a word that appears nowhere inside it — and rather than
leave you hunting for it, the detail line says `on the shelf as “No one named”`. It is the one
match a reader could never find by looking harder at the page, so it is the one the page has to
say out loud.
...
(`decisions/0014`). `scripts/make-mirror-vault.mjs` is still here and still refuses to finish
...
`design/0007`, `design/0013`, `decisions/0014`
